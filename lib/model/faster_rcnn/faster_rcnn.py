...
class _fasterRCNN(nn.Module):
    ...
    def forward(self, im_data, im_info, gt_boxes, num_boxes, ignore_class=-100):
        ...
        rois, rpn_loss_cls, rpn_loss_bbox = self.RCNN_rpn(
            base_feat, im_info, gt_boxes, num_boxes, ignore_class)
        ...
        if self.training:
            ...
            RCNN_loss_cls = F.cross_entropy(
                cls_score, rois_label, ignore_index=ignore_class)

            weights = (rois_label != ignore_class)

            # bounding box regression L1 loss
            RCNN_loss_bbox = _smooth_l1_loss(
                bbox_pred, rois_target, rois_inside_ws, rois_outside_ws, weights)
